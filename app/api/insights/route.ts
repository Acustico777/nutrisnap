...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: meals, error } = await supabase
      .from('meals')
      .select('consumed_at, meal_items(name, quantity, calories, category)')
      .eq('user_id', user.id)
    ...
    // Build daily series map: date string -> per-category calories
    const seriesMap: Map<string, Record<string, number>> = new Map();
    ...
    for (const meal of meals ?? []) {
      ...
      const dayCats = seriesMap.get(mealDate);

      for (const item of (meal.meal_items as Array<{ name: string; quantity: number; calories: number; category?: string }> ?? [])) {
        const cat = (item.category ?? 'other') as string;
        const kcal = (item.calories ?? 0) * (item.quantity ?? 1);
        ...
        if (dayCats) {
          dayCats[cat] = (dayCats[cat] ?? 0) + kcal;
        }
      }
    }
    ...
    const series = Array.from(seriesMap.entries()).map(([date, cats]) => {
      const dayTotal = Object.values(cats).reduce((a, b) => a + b, 0);
      return {
        date,
        label: dateLabel(date),
        categories: {
          vegetables: Math.round(cats['vegetables'] ?? 0),
          meat: Math.round(cats['meat'] ?? 0),
          fish: Math.round(cats['fish'] ?? 0),
          fruit: Math.round(cats['fruit'] ?? 0),
          other: Math.round(cats['other'] ?? 0),
        },
